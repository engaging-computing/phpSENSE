...
for($j=0; $j<$limits; $j++){
  $newest = $getExperiments_response['data'][$j]['meta']['session_count'];
  $next =  $getExperiments_response['data'][$j++]['meta']['session_count'];
  if($newest < $next){
      $successflag = 0;
      break;   
  }
...
for($j=0; $j<$limits; $j++){
  $newest = $getExperiments_response['data'][$j]['meta']['contrib_count'];
  $next =  $getExperiments_response['data'][$j++]['meta']['contrib_count'];
  if($newest < $next){
      $successflag = 0;
      break;   
  }
...
$getExperiments_response = getExperimentsTest($page, $limits, $query, $sort, $action);
for($j=0; $j<$limits; $j++){
  $date1 = date($getExperiment_response['data'][$j]['meta']['timecreated']);
  $date2 =  date($getExperiment_response['data'][$j++]['meta']['timecreated']);
  print_r($date1);
  print_r($date2);
  if($date2 < $date1){
      $successflag = 0;
