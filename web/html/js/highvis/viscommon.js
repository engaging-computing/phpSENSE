		...
		for( var i in data.sessions ){
            ...
            if(data.sessions[i].pictures[0] != null){
                for(var j in data.sessions[i].pictures){  
                    ...
                    if(j==0){
                        controls += '<a style="color:#' + (color[0]>>4).toString(16) + (color[1]>>4).toString(16) + (color[2]>>4).toString(16) + '" rel="prettyPhoto[gallery'+i+']" href="'+ link + '" title="'+description+'">' + data.sessions[i].metaData.name +'</a>';
                    } else {
                        controls += '<a rel="prettyPhoto[gallery'+i+']" href="'+ link + '" title="'+description+ '"/a>';
                    }                       
                }              
            } else {
                controls += data.sessions[i].metaData.name;
            }
